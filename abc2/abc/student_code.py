...
furkaasfasfafnimport numpy as np
import torch
...
class SimpleNet(nn.Module):
  ...
  def __init__(self, conv_op=nn.Conv2d, num_classes=100):
    ...
    self.features = nn.Sequential(
      ...
      conv_op(3, 64, kernel_size=7, stride=2, padding=3, bias = True),
      nn.BatchNorm2d(64, eps=1e-05, momentum=0.1, affine=True),
      nn.ReLU(inplace=True),
      conv_op(64, 64, kernel_size=7, stride=2, padding=3,bias = True),
      nn.BatchNorm2d(64, eps=1e-05, momentum=0.1, affine=True),
      nn.ReLU(inplace=True),
      conv_op(64, 64, kernel_size=7, stride=2, padding=3,bias = True),
      nn.BatchNorm2d(64, eps=1e-05, momentum=0.1, affine=True),
...
def get_train_transforms(normalize):
  ...
  train_transforms.append(transforms.RandomColor(0.3))
  train_transforms.append(transforms.RandomRotate(30))
